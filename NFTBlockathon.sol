...
abstract contract VRC725 is ERC165, IVRC725 {
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, data), "VRC725: transfer to non ERC721Receiver implementer");
    }
    ...
    function _safeMint(
        ...
        require(
            ...
            "VRC725: transfer to non ERC721Receiver implementer"
        );
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "VRC725: mint to the zero address");
        require(!_exists(tokenId), "VRC725: token already minted");
    ...
    function _checkOnERC721Received(
        ...
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(msg.sender, from, tokenId, data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("VRC725: transfer to non ERC721Receiver implementer");
                } else {
                    /// @solidity memory-safe-assembly
                    assembly {
 ...
 * account.
 */
abstract contract VRC725Enumerable is VRC725, IERC721Enumerable {
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, VRC725) returns (bool) {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < VRC725.balanceOf(owner), "VRC725Enumerable: owner index out of bounds");
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < VRC725Enumerable.totalSupply(), "VRC725Enumerable: global index out of bounds");
    ...
    function _beforeTokenTransfer(
        ...
        if (batchSize > 1) {
            ...
            revert("VRC725Enumerable: consecutive transfers not supported");
        }
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = VRC725.balanceOf(to);
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = VRC725.balanceOf(from) - 1;
...
abstract contract MinterRole {
    ...
    event MintersUpdated(address[] , bool[] status);
    ...
    function _setMinters(address[] memory minters, bool[] memory isActives) internal {
        for(uint256 i = 0; i < minters.length; i++) {
            isMinter[minters[i]] = isActives[i];
        }

        emit MintersUpdated(minters, isActives);
    }

    function setMinters(address[] memory minters, bool[] memory isActives) external virtual;
}
...
contract AdventureCollection is VRC725Enumerable, MinterRole {
    ...
    constructor(string memory name, string memory symbol, string memory initBaseURI, address issuer) {
        isMinter[issuer] = true;
        baseURI = initBaseURI;

        __VRC725_init(name, symbol, issuer);
    }

    function setMinters(address[] memory minters, bool[] memory isActives) external override onlyOwner {
        _setMinters(minters, isActives);
    }
    ...
    function mint(address owner) external onlyMinter {
        uint256 currentIndex = totalSupply();
        _safeMint(owner, currentIndex + 1);
    }

    function mintMulti(address owner, uint256 amount) external onlyMinter {
